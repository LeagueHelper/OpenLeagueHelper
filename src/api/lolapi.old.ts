...
class lolapi extends EventEmitter {
    ...
    _initProcessWatcher() {
        return lolapi.getLCUPathFromProcess().then((lcuPath) => {
            if (lcuPath) {
    ...
    _onFileCreated(path) {
        fs.readFile(path, 'utf8')
            .then((data) => {
                ...
                this.client.auth = `Basic ${Buffer.from(
    ...
    _initWebSocket() {
        const socket = new WebSocket(
            `wss://riot:${this.client.token}@localhost:${this.client.port}`,
            {
                headers: {
        ...
        socket.on('error', (data) => {
            console.log('DIOS LA PUTA MADRE CON WSS');
            //   sleep(30000).then(() => {
            //     this._initWebSocket();
            //     this._initSummoner()
            // });});
                ...
                .then((res) => {
                    try {
                        ...
                        list.forEach((onechampion) => {
                            if (
                                onechampion.ownership.owned ||
                                onechampion.freeToPlay
                            ) {
                                own.push(
                                    `${onechampion.id} ${onechampion.name}`
                                );
                            }
                        });
                        resolve(own);
